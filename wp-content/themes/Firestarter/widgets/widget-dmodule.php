...
function __construct()
{
    parent::__construct(
        false, // Base ID
        'Firestarter Homepage Module-D', // Name
...
$module4cat = $instance['categories'];/* Before widget (defined by themes). */
echo $before_widget; ?><?php $module4catbreaking = "cat=$module4cat"; ?><?php
...
<div class="middlewrapper"><?php query_posts('showposts=2&' . $module4catbreaking); ?> <!--original value is 4& -->
    <?php if (have_posts()) : ?>
        <?php while (have_posts()) : the_post(); ?>
            <div class="homeregularpost"><?php if (has_post_thumbnail()) { ?>
                <div class="homeregularimg">
                    <div class="homeregularimgfirst"><?php if (has_post_thumbnail()) {
                            the_post_thumbnail('channelthumb');
                        } ?></div><!-- /homeregularimgfirst --></div><!-- /homeregularimg -->
            <?php } ?>
            <div class="homeregulartitle">
            <h1><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h1>
            </div><!-- /homeregulartitle -->
            <div class="homepostsmeta"><?php the_time(get_option('date_format')); ?></div>
            <!-- /homepostsmeta -->
            <div class="homeregularexcerpt"><p><?php echo get_excerpt(200); ?></p></div>
            <!-- /homeregularexcerpt -->
            <div class="homepostsauthormeta">BY&nbsp; <?php the_author_posts_link(); ?></div>
            <!-- /homepostsauthormeta -->
            <div id="posttags">
                <?php the_tags('<p>', '', '</p>'); ?><!-- /homepostsinfo --></div>
            </div><!-- /homeregularpost --><?php endwhile; ?>
    <?php endif; ?></div><!-- /middlewrapper --><?php /* After widget (defined by themes). */
echo $after_widget;
}/**
