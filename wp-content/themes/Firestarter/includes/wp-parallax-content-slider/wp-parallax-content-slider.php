...

{
	...
	function __construct() {
		...
		load_plugin_textdomain( 'wp-parallax-content-slider', false, get_template_directory_uri() . '/includes/wp-parallax-content-slider/locale' );
...
add_action( 'wp_enqueue_scripts', 'wp_parallax_css' );

function wp_parallax_css() {

		wp_enqueue_style( 'wp-parallax-content-slider-css', get_template_directory_uri() . '/includes/wp-parallax-content-slider/css/style.css' );
	...
	public function activate( $network_wide ) {
		...
		$prlx_slider_settings = array(
				...
				'mode' => 'dynamic',					// Slider display mode (static / dynamic)

				'theme' => 'default',				// default / silver / retro / dark

				'bgincrement' => 0,				// increment the background position (i.e. parallax effect) when sliding (in px)

				'autoplay' => 1,					// slideshow auto switch ON (1) / OFF (0)
				...
				'title_max_chars' => 100,			// Max number of characters to display for a slide title
...

	{
		...
		$outputDynamic = "<div class='da-slider'>\n";
		...
		foreach( $myposts as $post ) :	setup_postdata($post);
			...
			if ( has_post_thumbnail() ) {
				...
				$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), 'newsthumb' );
...
$catlink = get_category_link($category[0]->term_id);
		...
		$outputSlide .= "<div class='da-img'><a href='".$link_article."'><img src='".$url_image."' alt='".$alt_image."' /></a>"."\n";

		$outputSlide .= "<div class='scrollboxcatname' style='background-color:".$saved_data.";'><a href='".$catlink."'>".$category[0]->cat_name."</a></div><div class='scrollboxposttitletrans'><div class='scrollboxposttitle'><a href='".$link_article."'>".$title."</a></div></div>"."\n";
...

	{
		...
		$page = add_theme_page( __( 'WP Parallax Content Slider', 'wp-parallax-content-slider' ),
