...
function zem_rp_append_posts(&$related_posts, $fetch_function_name, $limit)
{
    ...
    if ($num_missing_posts > 0) {
        function rp_exclude_ids()
        {
            return $p->ID;
        }

        $exclude_ids = array_map('rp_exclude_ids', $related_posts);
...
function zem_rp_head_resources()
{
    ...
    if (!empty($tags)) {
        function rp_urlencode($v)
        {
            return "\'" . urlencode(substr($v, strpos($v, '\'') + 1)) . "\'";
        }

        $post_tags = '[' . implode(', ', array_map('rp_urlencode', $tags)) . ']';
    } else {
        $post_tags = '[]';
    }
